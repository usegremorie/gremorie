...
const THEME_IMPORT_LINE = "@import '@shadng/core/theme.css';";
...
export async function initCommand(options: InitOptions = {}): Promise<void> {
  ...
  console.log();
  console.log(kleur.bold().cyan('shadng init'));
  ...
  const needs: string[] = [];
  if (!deps['@shadng/core']) needs.push('@shadng/core');
  ...
  if (!ctx.rootStylesPath) {
    ...
    if (current.includes("@shadng/core/theme.css")) {
      console.log(
        kleur.green('✓'),
  ...
  console.log(`  ${kleur.cyan('shadng add prompt-input')}`);
  console.log();
