...
export async function addCommand(
  ...
  console.log();
  console.log(kleur.bold().cyan(`shadng add ${name}`));
  console.log();
  ...
  if (!entry.available) {
    ...
    console.log(kleur.dim('Run'), kleur.cyan('shadng list'), kleur.dim('to see what ships today.'));
    console.log();
    process.exit(1);
  }
  ...
  console.log(
    kleur.dim('Docs:'),
    kleur.cyan(`https://shadng.dev/docs/components/${entry.name}`),
  );
  console.log();
}
