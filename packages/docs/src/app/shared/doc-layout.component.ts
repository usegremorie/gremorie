...
const NAV: NavSection[] = [
  ...
  },
  {
    ...
    categories: [
      ...
      {
        title: 'Layout',
        items: [{ label: 'Scroll Area', href: '/docs/components/scroll-area' }],
      },
      {
        title: 'Code',
        items: [],
    ...
    <div class="flex min-h-screen flex-col bg-background text-foreground">
      ...
      >
        ...
        >
          ...
          >
            <span
              class="rounded bg-foreground px-1.5 py-0.5 text-xs font-bold text-background"
              >SHAD</span
            >
      ...
      <div class="mx-auto flex w-full max-w-6xl flex-1 gap-8 px-4 py-8">
        <aside
          class="sticky top-20 hidden h-[calc(100vh-6rem)] w-60 shrink-0 overflow-y-auto pr-2 md:block"
        >
              ...
              <div class="flex flex-col gap-2">
                ...
                @if (section.categories) {
                  <div class="flex flex-col gap-3">
                    @for (
                      category of section.categories;
                      track category.title
                    ) {
                      <div class="flex flex-col gap-0.5">
                        <h3
                          class="flex items-center gap-2 px-2 pt-1 text-[11px] font-semibold uppercase tracking-wider text-foreground/70"
                        >
                          {{ category.title }}
                          @if (category.comingSoon) {
                            <span
                              class="rounded bg-muted px-1.5 py-0.5 text-[9px] font-medium uppercase tracking-wide text-muted-foreground"
                              >soon</span
                            >
                          }
                        </h3>
                        ...
                        @if (
                          category.items.length === 0 && category.comingSoon
                        ) {
                          <span
                            class="block px-2 py-1 text-xs italic text-muted-foreground/70"
                          >
                            Coming in v0.2
                          </span>
                        }
      ...
      <footer class="border-t border-border bg-muted/30 py-6">
        <div
          class="mx-auto flex max-w-6xl items-center justify-between px-4 text-sm text-muted-foreground"
        >
