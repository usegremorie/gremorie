...
```
@gremorie/ng-charts                 (pacote único)
├── @gremorie/ng-charts/headless    ← FASE 1: o "Recharts for Angular", sem visual
└── @gremorie/ng-charts             ← FASE 2: charts shadng (Card + tokens + tooltip/legend)

@gremorie/ng-charts-advanced        ← FUTURO/OPCIONAL: wrapper Unovis (sankey, network, mapas, canvas/N alto)
```
...
### Fase 1 — `@gremorie/ng-charts/headless` (mecânica, sem estilo)
...
### Fase 2 — `@gremorie/ng-charts` (styled, marca shadng)
...
- Documentar o threshold de N onde se recomenda `@gremorie/ng-charts-advanced` (Unovis/canvas) em vez de fingir que SVG resolve tudo.
...
- Pacote npm `@gremorie/ng-charts` publicado (+ `@gremorie/ng-charts-advanced` futuro).
...
- `@gremorie/ng-charts-advanced` / Unovis (futuro/opcional).
