...
export function GET() {
  ...
  lines.push('## Install');
  lines.push('');
  lines.push('Two ways to consume Gremorie — choose per project:');
  lines.push('');
  lines.push('1. npm packages (versioned, get updates with `npm update`):');
  lines.push('   - React, complete edition: `npm i @gremorie/react`');
  lines.push('     ```tsx');
  lines.push(
    '     // 1. Import the pre-compiled stylesheet ONCE, in your app entry',
  );
  lines.push(
    '     //    (app/layout.tsx or src/main.tsx). It ships tokens + all',
  );
  lines.push(
    '     //    component styles — no Tailwind/@theme/@source needed.',
  );
  lines.push('     import "@gremorie/react/styles.css";');
  lines.push(
    '     // 2. Import components (Next.js App Router: file needs "use client").',
  );
  lines.push(
    '     import { Button, Card, CardHeader, CardTitle, Message } from "@gremorie/react";',
  );
  lines.push('     ```');
