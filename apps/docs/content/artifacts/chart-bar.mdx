...
The Angular edition ships the `ChartArtifact` shell from `@gremorie/ng-artifacts`,
which wraps any `@gremorie/ng-data` chart with a chart ⇄ table toggle and
PNG / CSV downloads, plus the same accessible data table:
...
Pass `type` (`bar | area | line | pie | radar | radial | scatter`) to switch the
embedded chart, or a `valueKey` array for multi-series. The shell mirrors the
React `ChartArtifact` input for input.
...
Generated from the agnostic component contract (`@gremorie/contracts`), so the
React and Angular editions and this table never drift:
