...
<Callout type="info">
  This artifact supports partial schema. As the model streams code,
  Shiki re-highlights on each chunk so the user sees colorized output
  appear progressively. The copy button stays armed against the latest
  buffered value.
</Callout>
