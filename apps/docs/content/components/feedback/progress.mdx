...
The same bar across the full determinate range - `0`, `33`, `66`, and `100`. Pair every bar with its numeric value so the fill is never silent: at `0` the user sees the task is queued, at `100` that it is done. `value` is clamped between `0` and `max` (default `100`).
