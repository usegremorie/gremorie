  ...
  template: `
    <div class="flex items-center gap-4" aria-busy="true" aria-live="polite">
      ...
      <div class="flex flex-col gap-2">
        <gn-skeleton class="h-4 w-[200px]" />
        <gn-skeleton class="h-4 w-[160px]" />
      </div>
    </div>
  `,
})
...
The standard user-profile placeholder: a circular avatar beside two short text lines. Two lines feel honest; three is the upper bound before the placeholder starts to flicker on real data.
...
For card-grid layouts. Use `aspect-video` on the cover Skeleton so the placeholder height matches what the real `<img>` will reserve, then taper the two text lines to `w-3/4` and `w-1/2` so the block reads as a heading plus subtext.
...
Render one row per expected item rather than a single big block. Use a fixed count that matches the typical page size (5-10 rows). Overshooting feels like fake throughput; undershooting causes a jump when more rows arrive.
