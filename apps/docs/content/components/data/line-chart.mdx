...
<Tab value="Angular">
```bash
npx gremorie@latest add ng-line-chart
```
...
| `yAxis`     | `boolean`                                       | `true`      | Render the numeric Y axis. Set to `false` for a minimal, axis-free line (sparkline-style).                                                                                                       |
...
Set `dots` to render a marker at each data point. Reach for it when the data is sparse and you want each reading to be individually clickable / readable.
...
Set `type` to pick the interpolation curve - `"natural"` (default), `"monotone"`, `"linear"` (straight segments), or `"step"` (stepped edges). Use `"linear"` to read exact slopes; use `"step"` for discrete state changes.
...
Set `yAxis` to `false` to drop the numeric axis for a minimal, sparkline-style line that leans on the tooltip for exact values.
