...
<Callout type="info">
Slice colors come from each row's `fill` (e.g. `fill: "var(--chart-1)"`), not from `config`. The optional `config` maps each slice name to a `{ label }` and drives the labels shown in the tooltip.
</Callout>
...
```tsx
<PieChart data={data} config={config} nameKey="browser" dataKey="visitors" donut />
```
  ...
  return (
    <ChartContainer config={config} className="mx-auto aspect-square max-h-[260px]">
      <PieChart>
        ...
        <Pie data={data} dataKey={dataKey} nameKey={nameKey} stroke="var(--background)" strokeWidth={2}>
          {data.map((row) => (
            <Cell key={String(row[nameKey])} fill={colorByName[String(row[nameKey])]} />
          ))}
        </Pie>
      </PieChart>
    </ChartContainer>
