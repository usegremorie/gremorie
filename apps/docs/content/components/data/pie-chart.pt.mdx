...
<Callout type="info">
  As cores das fatias vêm do `fill` de cada linha (ex. `fill:
  "var(--chart-1)"`), não do `config`. O `config` opcional mapeia cada nome de
  fatia para um `{label}` e alimenta os labels mostrados no tooltip.
</Callout>
