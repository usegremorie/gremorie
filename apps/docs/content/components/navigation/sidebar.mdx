...
Pass `variant="floating"` to detach the panel from the viewport edge with a rounded border and shadow. It pairs naturally with `collapsible="icon"`. The other variants are `"sidebar"` (default, flush to the edge) and `"inset"` (which pushes the paired `<SidebarInset>` in as a rounded card).
