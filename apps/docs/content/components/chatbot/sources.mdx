...
`Sources` is the global citations panel for an assistant turn. Where [InlineCitation](/components/chatbot/inline-citation) attaches a citation to a specific phrase, `Sources` lists every document the answer relied on, collapsed by default with a count summary ("Used 4 sources").
...
<Tab value="Angular">
```bash
npx gremorie@latest add ng-sources
```
...
- [InlineCitation](/components/chatbot/inline-citation) - phrase-level citations inside the response
- [Message](/components/chatbot/message) - assistant turn that typically hosts the sources panel
- [Chain of Thought](/components/chatbot/chain-of-thought) - per-step search hits during reasoning
