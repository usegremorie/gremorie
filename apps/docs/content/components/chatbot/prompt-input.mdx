...
<Tab value="Angular">
```bash
npx gremorie@latest add ng-prompt-input
```
...
`PromptInput` is customized by composition, not configuration. There is no
`showAttachments` or `showVoice` prop, by design (the same contract as Vercel
AI Elements): every part is a component you include in the tree or leave out.
To remove the context section, the voice button or the attachments flow, you
delete the part from your code instead of toggling a flag. The recipes below
are complete, copyable starting points ordered from the smallest possible
composer to the full B2B surface.

### Minimal

The smallest valid composition: a body with the textarea and a footer with the
submit button. The empty `<span />` keeps the footer's `justify-between`
layout pushing submit to the right. Everything else is opt-in.
...
Adds the attach affordance and the chips row. `PromptInputAttachButton` opens
the file picker, and `PromptInputAttachments` renders one chip per attached
file (paste and drag-drop funnel into the same list). The standalone
`Attachments` row seeds the preview with sample files so you can see the chip
treatment per media type.
...
The Gremorie-standard business composition, the same surface the Assistant
block ships: an `@` add-context command palette plus a token-usage meter in
the header, Mode and Model selects with provider icons in the footer, and a
web search / attach / voice icon group next to the submit button. Every
action carries a tooltip.
