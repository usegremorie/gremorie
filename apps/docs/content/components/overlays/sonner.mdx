...
The Gremorie `Toaster` follows the document's `.dark` class automatically and ships with Gremorie icons for the `success`, `info`, `warning`, `error`, and `loading` toast types.
...
<Tab value="Angular">
```bash
npx gremorie@latest add ng-sonner
```
...
Wraps sonner's `Toaster` with Gremorie tokens and icons. Mount once per app.
