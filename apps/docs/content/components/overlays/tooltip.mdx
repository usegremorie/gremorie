...
<Tab value="Angular">
```bash
npx gremorie@latest add ng-tooltip
```
...
`TooltipContent` accepts a `side` prop (`top`, `right`, `bottom`, `left`) and flips automatically when there is no room. Hover or focus each trigger to see the tooltip and its arrow.
...
- [Button](/components/buttons/button) - the most common tooltip trigger.
