...
<Tab value="Angular">
```bash
npx gremorie@latest add ng-dropdown-menu
```
...
### Submenu with shortcuts and destructive action

The canonical row-actions menu: right-aligned shortcuts, a nested `DropdownMenuSub`, and the destructive variant for delete.
...
Toggle visible columns or filters without closing the menu. Each `DropdownMenuCheckboxItem` renders its own check indicator.
...
Pick a single value from a `DropdownMenuRadioGroup`. The selected item renders an auto-positioned bullet.
