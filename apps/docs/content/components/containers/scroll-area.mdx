...
The Angular edition is a standalone `gr-scroll-area` component that implements its own overlay scrollbar natively (signals + `ResizeObserver`, with no external dependency) - the native bar is hidden and a `--border`-colored pill thumb floats over the content, taking no layout space and fading in on hover, matching the React edition's default `type="hover"` behaviour.
