...
The Angular edition is a standalone `gr-scroll-area` component; the visible bar comes from the DS scrollbar token baseline in `@gremorie/tokens`, so there is no third-party scrollbar library or imports bundle to wire up.
