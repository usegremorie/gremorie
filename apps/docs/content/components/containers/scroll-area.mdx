...
<Tab value="npm">
```bash
npx gremorie@latest add rx-scroll-area
```
</Tab>
<Tab value="pnpm">
```bash
pnpm dlx gremorie@latest add rx-scroll-area
```
</Tab>
<Tab value="yarn">
```bash
yarn dlx gremorie@latest add rx-scroll-area
```
</Tab>
<Tab value="bun">
```bash
bunx --bun gremorie@latest add rx-scroll-area
```
</Tab>
</Tabs>
  ...
  <div className="flex w-max gap-3 p-4">
    {covers.map((cover) => (
      <figure key={cover.id} className="shrink-0">
        <img src={cover.src} alt={cover.alt} className="h-32 w-48 rounded-md object-cover" />
      </figure>
    ))}
  </div>
