...
A edição Angular é um componente standalone `gr-scroll-area` que renderiza uma scrollbar overlay via [`ngx-scrollbar`](https://www.npmjs.com/package/ngx-scrollbar) - a barra flutua sobre o conteúdo e não ocupa espaço de layout, igual à edição React - temada com os tokens do Gremorie (fina, arredondada, thumb `--border`).
