...
- [Canvas](/components/ai/canvas) - the surface that hosts edges via `edgeTypes`
- [Connection](/components/ai/connection) - the in-progress drag line shown before an edge is committed
- [Node](/components/ai/node) - the source and target of every edge
