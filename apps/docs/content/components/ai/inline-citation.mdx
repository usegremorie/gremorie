...
Use it for citations that belong to a specific phrase inside the assistant text. For a global "sources used" list at the end of a turn, use [Sources](/components/ai/sources) instead.
...
<Tab value="npm">
```bash
npx gremorie@latest add rx-inline-citation
```
</Tab>
<Tab value="pnpm">
```bash
pnpm dlx gremorie@latest add rx-inline-citation
```
</Tab>
<Tab value="yarn">
```bash
yarn dlx gremorie@latest add rx-inline-citation
```
</Tab>
<Tab value="bun">
```bash
bunx --bun gremorie@latest add rx-inline-citation
```
</Tab>
</Tabs>
...
<InlineCitationSource title="WCAG 2.2" url={url} description="...">
  <InlineCitationQuote>
    "Authentication processes that rely on cognitive function tests must have an alternative."
  </InlineCitationQuote>
</InlineCitationSource>
```
...
- [Sources](/components/ai/sources) - global "sources used" panel for a turn
- [Message](/components/ai/message) - the assistant turn that hosts the citation
- [Chain of Thought](/components/ai/chain-of-thought) - shares the search-results pattern for step-level citations
