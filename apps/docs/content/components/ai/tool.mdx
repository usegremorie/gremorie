...
import { ToolRunningPreview, ToolCompletedPreview, ToolErrorPreview, ToolApprovalPreview } from "@/components/preview/ai/chatbot-rest";
...
<Tab value="npm">
```bash
npx gremorie@latest add rx-tool
```
</Tab>
<Tab value="pnpm">
```bash
pnpm dlx gremorie@latest add rx-tool
```
</Tab>
<Tab value="yarn">
```bash
yarn dlx gremorie@latest add rx-tool
```
</Tab>
<Tab value="bun">
```bash
bunx --bun gremorie@latest add rx-tool
```
</Tab>
</Tabs>
...
<Tab value="Angular">
<Callout type="info">
Angular edition planned for Phase 5h. The AI category in Angular currently only ships PromptInput. Use the React edition for now, or open an issue to prioritize this primitive.
</Callout>
</Tab>
</Tabs>
...
For dangerous tools (file writes, deletions, MCP calls with side effects), surface approval state before the call runs. Pair with [`Confirmation`](/components/ai/confirmation) for the actual accept / reject UI.
...
- [Confirmation](/components/ai/confirmation) - approval dialog for `approval-requested` state
- [Code Block](/components/ai/code-block) - the underlying surface for input and output rendering
- [Task](/components/ai/task) - higher-level task / subtask block when one tool call is part of a longer plan
