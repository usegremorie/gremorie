...
<Tab value="npm">
```bash
npx gremorie@latest add rx-image
```
</Tab>
<Tab value="pnpm">
```bash
pnpm dlx gremorie@latest add rx-image
```
</Tab>
<Tab value="yarn">
```bash
yarn dlx gremorie@latest add rx-image
```
</Tab>
<Tab value="bun">
```bash
bunx --bun gremorie@latest add rx-image
```
</Tab>
</Tabs>
...
<Artifact>
  ...
  <ArtifactContent>
    <Image base64={logo.base64} mediaType={logo.mediaType} alt="Generated logo" />
  </ArtifactContent>
</Artifact>
```
...
- [Artifact](/components/ai/artifact) - the card chrome around image artifacts
- [Message](/components/ai/message) - the chat turn that may inline an image
- [Open in Chat](/components/ai/open-in-chat) - send a generated image back into the conversation
