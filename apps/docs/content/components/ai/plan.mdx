...
Use it for any flow where the model proposes a plan before acting (multi-step agents, deploy bots, codegen). For granular per-step status icons and search citations, layer [Chain of Thought](/components/ai/chain-of-thought) or [Task](/components/ai/task) inside `PlanContent`.
...
- [Chain of Thought](/components/ai/chain-of-thought) - per-step status inside the plan body
- [Task](/components/ai/task) - file-level breakdown for a single step
- [Confirmation](/components/ai/confirmation) - approval prompt before running the plan
- [Shimmer](/components/ai/shimmer) - the streaming text effect used by `PlanTitle` / `PlanDescription`
