...
<Tab value="Angular">
```bash
npx gremorie@latest add ng-separator
```
...
The default horizontal orientation renders `h-px w-full`, dividing stacked sections. This is the most common case and is also shown in the Preview above.
...
Vertical separators need an explicit height (`h-4` here) because their default `h-full` only fills a constrained parent.
