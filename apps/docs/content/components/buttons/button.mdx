...
When an icon button sits next to another control, match height steps through the size variants (`icon-sm` with a `size="sm"` select or button, `icon` with the defaults) instead of forcing heights with classes. See [Action rows and size pairing](/components/buttons/button-group#action-rows-and-size-pairing).
...
- [Button Group](/components/buttons/button-group) - join several buttons into a single border-shared cluster
- [Input Group](/components/text/input-group) - embed `InputGroupButton` inside an input
- [Toggle](/components/buttons/toggle) - two-state button (`aria-pressed`) for stateful actions
- [Field](/components/form) - composes Button with `FormControl` for submit affordances
