...
<Tab value="Angular">
```bash
npx gremorie@latest add ng-select
```
...
The size variant owns the trigger height via `data-[size=...]` selectors, so never force a height with `className` - a manual `h-8` on a default-size trigger loses to the variant's `h-9`. Next to an icon `Button`, pair `size="sm"` with `size="icon-sm"` (both 32px) or the defaults with `size="icon"` (both 36px). See [Action rows and size pairing](/components/buttons/button-group#action-rows-and-size-pairing).
...
Disable the whole control with `disabled` on `Select`, or lock out a single option with `disabled` on `SelectItem`. Disabled items are announced as disabled and skipped by keyboard navigation.
...
- [Button Group](/components/buttons/button-group) - mix `SelectTrigger` with `Button`s in the same cluster
- [Toggle Group](/components/buttons/toggle-group) - icon-led single-select for 2-5 options
- [Radio Group](/components/selection/radio-group) - label-led single-select that always shows all options
- [Form](/components/form) - wire Select into react-hook-form via `FormField` + `Controller`
