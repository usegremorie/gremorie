...
<Tab value="Angular">
```bash
npx gremorie@latest add ng-slider
```
...
Use a larger `step` to snap to discrete values like quantity, rating, or font size.
...
`disabled` removes the thumbs from the tab order and drops opacity to 50%.
...
- [Input](/components/text/input) - use for exact numeric values
- [Form](/components/form) - wire Slider into react-hook-form
