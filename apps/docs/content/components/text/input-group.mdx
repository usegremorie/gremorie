...
<Tab value="Angular">
```bash
npx gremorie@latest add ng-input-group
```
...
Place an `InputGroupAddon` on each side - `inline-start` for a leading icon and `inline-end` for a trailing one. Both click through to focus the input.

<ComponentPreview name="forms/input-group-icon">
  <InputGroupIconPreview />
</ComponentPreview>

### Prefix label with trailing button

The leading addon shows a static label that clicks-through to focus the input; the trailing addon hosts an `InputGroupButton`.
...
- [Input](/components/text/input) - the leaf control without addons
- [Textarea](/components/text/textarea) - multi-line counterpart
- [Button](/components/buttons/button) - the base for `InputGroupButton`
- [Form](/components/form) - wires the whole group to react-hook-form
- [Field](/components/form) - composes label + group + description + message
