...
Pair with [`Tool`](/components/chatbot/tool). `ToolInput` and `ToolOutput` both render through `CodeBlock`, so JSON tool calls inherit the same highlighting and theming.
...
- [Artifact](/components/code/artifact) - the card chrome that hosts a `CodeBlock` for generated code artifacts
- [Tool](/components/chatbot/tool) - uses `CodeBlock` internally to render tool input and output
- [Web Preview](/components/code/web-preview) - the iframe surface paired with `CodeBlock` for live HTML preview
