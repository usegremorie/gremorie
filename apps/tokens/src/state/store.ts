...
export const useProject = create<ProjectState & ProjectActions>((set, get) => ({
  project: null, graph: null, pendingChanges: [],
  selectedTheme: "default", selectedMode: "light",
  loading: false, error: null, gitStatus: null,
  ...
  async load() {
    ...
    try {
      ...
      set({ project: data.project, graph: data.graph, gitStatus: gs, loading: false });
    } catch (e: any) {
  ...
  toggleMode() { set({ selectedMode: get().selectedMode === "light" ? "dark" : "light" }); },
  setTheme(t) { set({ selectedTheme: t }); },

  queueChange(c) { set({ pendingChanges: [...get().pendingChanges, c] }); },
  ...
  async apply() {
    ...
    try {
      ...
      set({ pendingChanges: [] });
      await get().load();
      return { ok: true, message: `Applied ${body.changeCount} change(s) to ${body.written}` };
    } catch (e: any) {
