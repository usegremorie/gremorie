...
export function SemanticColumn() {
  ...
  const semantics = Object.values(graph.semantics).filter(
    (s: any) => s.theme === selectedTheme && s.mode === selectedMode,
  );
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <ul className="flex flex-col gap-1 max-h-[480px] overflow-auto">
        {semantics.map((s: any) => {
          const targetName =
            s.value.kind === 'reference' ? s.value.targetName : null;
          const target = targetName
            ? graph.primitives[`${s.theme}::${s.mode}::${targetName}`]
            : null;
          return (
            <li
              key={s.name}
              className="flex flex-col gap-1 rounded-md border border-input px-2 py-1"
            >
