...
const rxSet = new Set(
  readdirSync(path.join(REG, 'rx')).map((f) => f.replace(/\.json$/, '')),
);
const ngSet = new Set(
  readdirSync(path.join(REG, 'ng')).map((f) => f.replace(/\.json$/, '')),
);
...
function mcpFor(node) {
  ...
  if (tab === 'components') {
    if (kind === 'tab')
      return { tool: 'list_components', call: 'list_components()' };
    if (kind === 'category')
      return {
        tool: 'search_components',
        call: `search_components(category=${q(slug)})`,
      };
  ...
  if (tab === 'artifacts') {
    if (kind === 'tab')
      return {
        tool: 'search_components',
        call: `search_components(query="artifact")`,
      };
    return {
      tool: 'get_component',
      call: `get_component(${q('artifact-' + slug)})`,
    };
  }
  if (tab === 'blocks') {
    if (kind === 'tab')
      return {
        tool: 'search_components',
        call: `search_components(query="block")`,
      };
    return { tool: 'get_block', call: `get_block(${q('block-' + slug)})` };
  }
  if (tab === 'corpus') {
    if (kind === 'tab')
      return { tool: 'get_guidelines', call: 'get_guidelines()' };
  ...
  parts
    ...
    .map((p) => {
      ...
      if (p.link != null)
        return {
          type: 'text',
          text: { content: p.label, link: { url: p.link } },
        };
...
const li = (...parts) => ({
  ...
  bulleted_list_item: { rich_text: rt(...parts) },
});
...
function pageBlocks(r) {
  ...
  if (r.kind === 'item' && r.tab === 'components') {
    b.push(
      li(
        'MCP — ',
        code(`get_component("rx-${r.slug}")`),
        ...(ngPub
          ? [' · Angular ', code(`get_component("ng-${r.slug}")`)]
          : []),
      ),
    );
    b.push(
      li(
        'Registry / CLI — ',
        code(`npx gremorie add rx-${r.slug}`),
        ...(ngPub ? [' · ', code(`ng-${r.slug}`)] : []),
      ),
    );
    b.push(
      li(
        ...
        code('@gremorie/react'),
        ' → ',
        code(`import { ${pascal(r.title)} }`),
        ...(ngPub ? [' · ', code('@gremorie/angular')] : []),
      ),
    );
  } else if (r.kind === 'item' && r.tab === 'artifacts') {
    b.push(li('MCP — ', code(`get_component("artifact-${r.slug}")`)));
    b.push(
      li('Registry / CLI — ', code(`npx gremorie add artifact-${r.slug}`)),
    );
...
for (const tab of TABS) {
  ...
  for (const page of meta.pages) {
    ...
    if (cmeta && childPages.length > 0) {
      ...
      rows.push({
        key: cKey,
        parentKey: parentForThis,
        kind: 'category',
        tab,
        slug: page,
        title: cmeta?.title ?? titleCase(page),
        tipo: 'Categoria',
        doc: `${ORIGIN}/${rel}`,
      });
      let grupo = null;
      for (const sub of cmeta?.pages ?? []) {
        ...
        rows.push({
          ...
          tab,
          slug: sub,
          title: pageTitle(rel, sub),
          tipo: tab === 'components' ? 'Componente' : 'Item',
          categoria:
            tab === 'components' ? (cmeta?.title ?? titleCase(page)) : null,
...
console.log(
  `Árvore desejada: ${rows.length} linhas (` +
...
if (!TOKEN || !DB_ID) {
  ...
  console.error(
    APPLY
      ? 'Abortando (--apply exige credenciais).'
      : 'Seguindo só com o plano local (dry-run).',
  );
...
async function main() {
  ...
  console.log(
    WITH_MCP
      ? 'Schema: Key + MCP garantidos.'
      : 'Schema: Key garantido (use --mcp p/ colunas MCP).',
  );
  ...
  console.log(
    `Notion atual: ${existing.size + [...byDoc.values()].filter((r) => !r.key).length} linhas lidas.`,
  );
  ...
  const text = (v) =>
    v ? { rich_text: [{ text: { content: String(v) } }] } : { rich_text: [] };
  const sel = (v) =>
    v && v !== '—' ? { select: { name: v } } : { select: null };
  ...
  for (const r of rows) {
    ...
    if (r.categoria !== undefined && r.categoria)
      properties.Categoria = { select: { name: r.categoria } };
    if (r.grupo) properties.Grupo = { select: { name: r.grupo } };
    if (r.react !== null && r.react !== undefined)
      properties.React = {
        select: { name: resolveStatus(r.react, cur?.react) ?? r.react },
      };
    if (r.angular !== null && r.angular !== undefined)
      properties.Angular = {
        select: { name: resolveStatus(r.angular, cur?.angular) ?? r.angular },
      };
    if (r.npm !== null && r.npm !== undefined)
      properties.npm = { checkbox: !!r.npm };
    if (r.registry !== null && r.registry !== undefined)
      properties.Registry = { checkbox: !!r.registry };
    if (WITH_MCP) {
      properties['MCP tool'] = sel(r.mcpTool);
    }
    ...
    if (cur) {
      ...
      if (APPLY) {
        await throttle();
        await notion.pages.update({ page_id: cur.id, properties });
      }
      updated++;
    } else {
      if (APPLY) {
        ...
        const pg = await notion.pages.create({
          ...
          icon: r.icon ? { type: 'emoji', emoji: r.icon } : undefined,
          properties,
        });
        keyToId.set(r.key, pg.id);
      } else {
        keyToId.set(r.key, `(novo:${r.key})`);
      }
      created++;
    }
  }
  ...
  for (const r of rows) {
    ...
    if (APPLY && childId && parentId && !String(childId).startsWith('(novo')) {
      ...
      await notion.pages.update({
        page_id: childId,
        properties: { 'item principal': { relation: [{ id: parentId }] } },
      });
    }
    rel++;
  }
  ...
  if (PAGES) {
    ...
    for (const r of rows) {
      ...
      if (APPLY) {
        await throttle();
        const kids = await notion.blocks.children.list({
          block_id: id,
          page_size: 100,
        });
